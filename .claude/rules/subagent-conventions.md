...
- Build: `turbo build` (all packages) or `bun run build` in `packages/varp/`.
...
**Varp components** (all in `packages/varp/`): `shared` = `src/shared/`, `manifest` = `src/manifest/`, `plan` = `src/plan/`, `scheduler` = `src/scheduler/`, `enforcement` = `src/enforcement/`, `analysis` = `src/analysis/`, `execution` = `src/execution/`, `mcp` = `src/mcp/`, `cli` = `src/cli/`, `skills` = `skills/`, `hooks` = `hooks/`. Domain components import shared types via `#shared/*` alias. Skills/hooks depend on manifest.
