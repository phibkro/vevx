...
describe("executeAuditPlan", () => {
  ...
  it("deduplicates overlapping findings across waves", async () => {
    ...
    expect(report.findings[0].effectiveConfidence).toBeGreaterThan(FINDING_MISSING_AUTH.confidence);
  });
  ...
  it("handles task failures gracefully", async () => {
    ...
    const failedCoverage = report.coverage.entries.find((e) => e.ruleId === "SEC-01" && !e.checked);
