...
describe('generatePlan', () => {
  ...
  it('matches rules to relevant files', () => {
    ...
    const apiTask = wave1.find(t =>
      t.component === 'src/api' && t.rules.includes('BAC-01')
    );
    expect(apiTask).toBeDefined();

    // DB should be checked against INJ-01
    const dbTask = wave1.find(t =>
      t.component === 'src/db' && t.rules.includes('INJ-01')
    );
