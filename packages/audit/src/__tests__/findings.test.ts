...
describe('deduplicateFindings', () => {
  ...
  it('sorts output by severity then confidence', () => {
    const results: AuditTaskResult[] = [
      makeTaskResult({
        findings: [
          makeFinding({ ruleId: 'LOW-01', severity: 'low', confidence: 0.9, locations: [{ file: 'a.ts', startLine: 1 }] }),
          makeFinding({ ruleId: 'CRIT-01', severity: 'critical', confidence: 0.7, locations: [{ file: 'b.ts', startLine: 1 }] }),
          makeFinding({ ruleId: 'HIGH-01', severity: 'high', confidence: 0.8, locations: [{ file: 'c.ts', startLine: 1 }] }),
        ],
      }),
    ];
...
describe('summarizeFindings', () => {
  it('counts findings by severity', () => {
    const deduped = deduplicateFindings([
      makeTaskResult({
        findings: [
          makeFinding({ ruleId: 'A', severity: 'critical', locations: [{ file: 'a.ts', startLine: 1 }] }),
          makeFinding({ ruleId: 'B', severity: 'critical', locations: [{ file: 'b.ts', startLine: 1 }] }),
          makeFinding({ ruleId: 'C', severity: 'high', locations: [{ file: 'c.ts', startLine: 1 }] }),
          makeFinding({ ruleId: 'D', severity: 'medium', locations: [{ file: 'd.ts', startLine: 1 }] }),
          makeFinding({ ruleId: 'E', severity: 'low', locations: [{ file: 'e.ts', startLine: 1 }] }),
          makeFinding({ ruleId: 'F', severity: 'informational', locations: [{ file: 'f.ts', startLine: 1 }] }),
        ],
      }),
    ]);
