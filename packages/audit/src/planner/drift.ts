 ...
 */
export function diffReports(baseline: ComplianceReport, current: ComplianceReport): DriftReport {
...
export function generateDriftMarkdown(drift: DriftReport): string {
  ...
  const trendEmoji =
    drift.summary.trend === "improving" ? "📉" : drift.summary.trend === "regressing" ? "📈" : "➡️";
  ...
  if (drift.new.length > 0) {
    ...
    for (const change of drift.new) {
      ...
      lines.push(
        `- **[${f.severity.toUpperCase()}]** \`${f.ruleId}\`: ${f.title} (${loc?.file}:${loc?.startLine})`,
      );
    }
    lines.push("");
  }
