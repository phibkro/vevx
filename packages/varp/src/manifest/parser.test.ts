...
describe("parseManifest", () => {
  test("parses the project's own varp.yaml", () => {
    ...
    expect(manifest.components.shared.path).toBe(resolve(PROJECT_ROOT, "packages/varp/src/shared"));
    expect(manifest.components.shared.stability).toBe("stable");
    expect(manifest.components.mcp).toBeDefined();
    expect(manifest.components.mcp.path).toBe(resolve(PROJECT_ROOT, "packages/varp/src/mcp"));
    ...
    expect(manifest.components.manifest.path).toBe(
      resolve(PROJECT_ROOT, "packages/varp/src/manifest"),
    );
    expect(manifest.components.manifest.deps).toEqual(["shared"]);

    // cli depends on explicit component names
