...
import type { DocumentSymbol, SemanticToken } from "./Lsp.js";
...
describe("isExported (text scanning)", () => {
  ...
  for (const [name] of Object.entries(expectedExports)) {
    const idx = lines.findIndex(
      (l) => l.includes(`function ${name}`) || l.includes(`const ${name}`) || l.includes(`class ${name}`) || l.includes(`interface ${name}`) || l.includes(`type ${name}`),
    );
...
describe.skipIf(!hasLsp)("Export detection spike (LSP integration)", () => {
  ...
  test("isExported matches all symbols from documentSymbol", async () => {
    ...
    const notExported = new Set(["helper", "INTERNAL", "InternalService", "InternalConfig", "InternalId"]);
