...
- **Tests**: Co-located with source. `*.test.ts` = interface tests (95% aggregate coverage enforced), `*.integration.test.ts` = external service tests (no threshold). Run concurrently (`--concurrent`). See `.claude/rules/testing.md` for conventions.
