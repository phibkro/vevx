...
- **Tests**: Co-located with source (`*.test.ts`). Run concurrently (`--concurrent`). See `.claude/rules/testing.md` for patterns and anti-patterns. Use `withTempDir` for filesystem tests, `test.serial` for `process.env` mutations. Integration tests use `InMemoryTransport` + `Client`.
